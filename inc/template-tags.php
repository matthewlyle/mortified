...
function hyphen_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && hyphen_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'hyphen' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'hyphen' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'hyphen' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
